...
import memberDetails.PersonConstants;
...
public class FileImplementation {
	...
	public ArrayList<Person> getAll(){
		try {
			...
			for (int i = 0; i < count; i++) {
				...
				personRecordList.add(new Person(id, accessType, name, lastName, email, number, dateJoined, address));
	...
	public Person getMember(int byIndex) {
		try {
			...
			Date dateJoined = java.text.DateFormat.getDateInstance().parse(readString(in,PersonConstants.DATE_JOINED_SIZE));
			in.close();
			return new Person(id, accessType, name, lastName, email, number, dateJoined, address);
